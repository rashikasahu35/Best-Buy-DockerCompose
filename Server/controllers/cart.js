...
exports.resetCart = catchAsyncError(async (userId) => {
...
exports.getCart = catchAsyncError(async (req, res) => {
    ...
    else throw new Error("Cart does not exist", 404);
});
...
exports.deleteCart = catchAsyncError(async (req, res) => {
    ...
    if (response) res.status(200).json({ response, message : "Cart deleted successfully" });
    else
        res.status(404).json({
            message:
                "Cart does not exist or user does not have anything in cart",
        });
});
...
exports.updateCart = catchAsyncError(async (req, res) => {
    ...
    const response = await Cart.findByIdAndUpdate(id, updatedCart, {
