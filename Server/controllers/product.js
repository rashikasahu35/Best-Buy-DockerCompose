...
exports.createProduct = catchAsyncError(async (req, res) => {
    ...
    await Promise.all(images?.map(async (img) => {
        const { public_id, secure_url } = await uploadImage(img, "products");
...
exports.updateProduct = catchAsyncError(async (req, res) => {
    ...
    if(images){
        ...
        const newImages = images.filter((image) => !(image.public_id && image.url))
        const existingImages = images.filter((image) => (image.public_id && image.url))
        const removedImages = []
        ...
        await Promise.all(newImages.map(async (image) => {
            const { public_id, secure_url } = await uploadImage(image, "products");
...
exports.deleteProduct = catchAsyncError(async (req, res) => {
    ...
    if (response.deletedCount === 1 && review) {
        res.status(200).json({ message: 'Product deleted successfully' });
    } else {
        if(response.deletedCount === 1 && !review ){
            throw new Error("Product deleted, but product review does not exist/deleted", 500);
        }
        else{
